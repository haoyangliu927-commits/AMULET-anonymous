...
def build_pca_initialization(
    *,
    ...
    if pcs is None or pcs.shape[1] < embed_dim:
        ...
        if x.shape[1] < num_genes:
            raise ValueError(f"adata has only {x.shape[1]} genes, but num_genes={num_genes}.")
        gene_matrix = x[:, :num_genes].T
        gene_matrix = gene_matrix - gene_matrix.mean(axis=1, keepdims=True)
        u, singular_values, _ = np.linalg.svd(gene_matrix, full_matrices=False)
        pcs = u[:, :embed_dim] * singular_values[:embed_dim]
        if pcs.shape[1] < embed_dim:
            pad = np.zeros((pcs.shape[0], embed_dim - pcs.shape[1]), dtype=pcs.dtype)
...
def run_supervised_hyperedges(
    *,
    ...
    exclude_garbage_from_relation_loss: bool = True,
    epochs: int = 10000,
    lr: float = 0.016,
    ...
    directed_inclusion_mask = (
        ...
        else _as_bool_mask(directed_inclusion_mask, name="directed_inclusion_mask")
    )
    inferred_num_genes = _infer_num_genes(
        pos_mask,
        neg_mask,
        partial_pos_mask,
        directed_inclusion_mask,
